...
//! sends when asked for its own format, what `MTLPixelFormat.bgra8Unorm` reads,
//! and what a `CGBitmapInfo` of `byteOrder32Little | alphaNoneSkipFirst` reads.
//! Nothing swizzles anywhere on this path.
//!
...
//! buffer is a different size and its texture has to be made again, and the
...
impl Framebuffer {
    ...
    /// time is looking at a new size and must make its texture again.
