//! The desktop's sound, between the session that decodes it and the Mac's
//! audio device that plays it.
//!
//! The session asks for one format — signed 16-bit stereo at 48 kHz, which is
//! what a Mac's output runs at and so needs no resampling on either side — and
//! decodes every FLAC frame into [`Playback`] as it arrives. The device takes
