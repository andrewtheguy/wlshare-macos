//! The C ABI the app calls, and the only place in this crate with `unsafe` in it.
//!
...
//!   Copy what you need out of it — upload it to a texture — and do not keep it.
//! - A callback runs with a lock held, so it must not call back into this API
//!   and must not wait for anything.
//!
//! `include/wlshare_client.h` declares all of it, and
//! `tests/header_matches.rs` fails if the two ever drift apart.
...
pub struct WlshareFrame {
    /// `width * height * 4` bytes of `B, G, R, X`, `stride` bytes a row —
    /// `MTLPixelFormat.bgra8Unorm` exactly. Null for a framebuffer with no
    /// pixels, which is a session before its ServerInit.
    pub pixels: *const u8,
    pub width: u32,
    pub height: u32,
    pub stride: u32,
    /// Which framebuffer this is. A number the window has not seen before is a
    /// framebuffer of a new size, whose texture has to be made again.
    pub generation: u64,
...
/// # Safety
/// `client` is live or null, and `out` points at a [`WlshareStatus`].
#[unsafe(no_mangle)]
...
/// Why the session ended, into `out`. Returns its length, which is 0 for a
/// session that has not failed.
///
/// # Safety
/// `out` has room for `cap` bytes, or `cap` is 0.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn wlshare_client_error(client: *const Client, out: *mut c_char, cap: usize) -> usize {
    let Some(client) = (unsafe { client.as_ref() }) else { return 0 };
    unsafe { copy_out(client.status().error.as_deref().unwrap_or_default(), out, cap) }
}

/// The desktop's name, into `out`. Returns its length.
///
/// # Safety
/// `out` has room for `cap` bytes, or `cap` is 0.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn wlshare_client_name(client: *const Client, out: *mut c_char, cap: usize) -> usize {
    let Some(client) = (unsafe { client.as_ref() }) else { return 0 };
    unsafe { copy_out(&client.status().name, out, cap) }
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn wlshare_client_wheel(client: *const Client, dx: f64, dy: f64, precise: bool, out: *mut u8, cap: usize) -> usize {
    let Some(client) = (unsafe { client.as_ref() }) else { return 0 };
    let notches = client.wheel(dx, dy, precise);
    let room = notches.len().min(cap);
    if !out.is_null() && room > 0 {
        unsafe { std::ptr::copy_nonoverlapping(notches.as_ptr(), out, room) };
    }
    room
}
...
#[unsafe(no_mangle)]
pub extern "C" fn wlshare_keysym(key_code: u16, character: u32) -> u32 {
    crate::keysym::keysym(key_code, char::from_u32(character).filter(|c| *c != '\0')).unwrap_or(0)
}
