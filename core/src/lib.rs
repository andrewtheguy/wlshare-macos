...
//!
//! The macOS app owns a window, a Metal texture and the events AppKit hands it.
//! This crate owns everything else: the socket, the RFB session, the decoders,
//! the framebuffer they write into, and the tables that say what a Mac key or a
//! Mac scroll is on the wire. Nothing here knows what AppKit is, which is what
//! lets all of it be tested on a machine that has never seen it.
//!
...
//! buffer the Mac's audio device takes from through [`Client::read_audio`], on
//! its own clock.
//!
//! **Scope.** Screen — as wlshare's VP9 stream or as exact ZRLE, whichever the
//! window chose — keyboard, pointer, retina, the clipboard and the desktop's
//! sound. Camera, microphone and picking an output are wlshare
...
pub use wheel::{WHEEL_DOWN, WHEEL_LEFT, WHEEL_RIGHT, WHEEL_UP, Wheel};
...
impl Client {
    ...
    /// The window's backing store changed: its size in device pixels, or the
    /// scale it draws at because it moved to another screen. The desktop is
    /// asked to follow once the change settles.
